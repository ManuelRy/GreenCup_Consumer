                        ...
                        <div class="card border-0 shadow-sm bg-gradient-primary text-white text-center rounded-4">
                            <div class="card-body py-5">
                                <div class="display-2 fw-bold mb-2 animate-number">
                                    {{ number_format($availablePoints ?? 1247) }}
                                </div>
                            ...
                            <div class="card-body text-center">
                                ...
                                <div class="position-relative d-inline-block mb-4">
                                    <svg class="donut-chart" viewBox="0 0 100 100" style="width: 180px; height: 180px;">
                                        @if ($total > 0)
                                            <!-- Background circle -->
                                            <circle cx="50" cy="50" r="40" fill="none" stroke="#f0f0f0"
                                                stroke-width="12" />
                                            ...
                                            @if ($pointsOut > 0)
                                                <circle cx="50" cy="50" r="40" fill="none" stroke="#ef4444"
                                                    ...
                                                    stroke-dashoffset="-{{ $pointsInLength }}" transform="rotate(-90 50 50)"
                                                    class="animate-chart" />
                                            @endif
                                        @else
                                            <!-- Empty state circle -->
                                            <circle cx="50" cy="50" r="40" fill="none" stroke="#e0e0e0"
                                                stroke-width="12" />
                                        @endif
                                    </svg>
                                ...
                                <div class="card border-0 shadow-sm h-100 rounded-3">
                                    <div class="card-body">
                                        ...
                                        <div class="h3 fw-bold text-success mb-2">
                                            {{ number_format($monthlyData['points_in'] ?? 0) }}
                                        </div>
                                        <small class="text-muted">
                                            @if (($monthlyData['prev_points_in'] ?? 0) > 0)
                                                {{ number_format($monthlyData['prev_points_in']) }} pts in
                                                {{ $monthlyData['prev_month_name'] ?? 'last month' }}
                                            @else
                                                This Month
                                            @endif
                                        </small>
                                ...
                                <div class="card border-0 shadow-sm h-100 rounded-3">
                                    <div class="card-body">
                                        ...
                                        <div class="h3 fw-bold text-danger mb-2">
                                            {{ number_format($monthlyData['points_out'] ?? 0) }}
                                        </div>
                                        <small class="text-muted">
                                            @if (($monthlyData['prev_points_out'] ?? 0) > 0)
                                                {{ number_format($monthlyData['prev_points_out']) }} pts in
                                                {{ $monthlyData['prev_month_name'] ?? 'last month' }}
                                            @else
                                                This Month
                                            @endif
                                ...
                                <div class="row g-3">
                                    <div class="col-6 col-lg-3">
                                        <a href="{{ route('account') }}"
                                            class="btn btn-outline-primary w-100 py-3 text-decoration-none action-btn">
                                            <div class="fs-2 mb-2">
                                    ...
                                    <div class="col-6 col-lg-3">
                                        <a href="{{ route('scan.receipt') }}"
                                            class="btn btn-primary w-100 py-3 text-decoration-none action-btn">
                                            <div class="fs-2 mb-2">
                                    ...
                                    <div class="col-6 col-lg-3">
                                        <a href="{{ route('report.index') }}"
                                            class="btn btn-outline-warning w-100 py-3 text-decoration-none action-btn">
                                            <div class="fs-2 mb-2">
                                                <i class="fas fa-exclamation-triangle"></i>
                                            </div>
                                            <div class="fw-semibold small">Report Issue</div>
                                        </a>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card-body">
                                @forelse($recentActivity as $activity)
                                    <div class="d-flex align-items-start py-3 border-bottom border-light activity-item">
                                        ...
                                        <div class="text-end">
                                            <span
                                                class="fw-bold {{ $activity->type === 'earn' ? 'text-success' : 'text-danger' }}">
                                                {{ $activity->type === 'earn' ? '+' : '-' }}{{ number_format($activity->points) }}
                                                pts
                                            </span>
                                        </div>
                                    </div>
