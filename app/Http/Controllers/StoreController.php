    ...
    private function getFeedPosts($page = 1, $perPage = 20, $sellerId = null)
    {
        try {
            ...
            $query = DB::table('seller_photos as sp')
                ->join('sellers as s', 's.id', '=', 'sp.seller_id')
                ->where('s.is_active', true)
                ->whereRaw('(sp.photo_caption IS NULL OR sp.photo_caption NOT LIKE ? AND sp.photo_caption NOT LIKE ?)', ['[FROZEN]%', '[frozen]%'])
                ->select([
                    'sp.id',
                    'sp.photo_url',
                    'sp.caption',
        ...
        try {
            ...
            if (DB::getSchemaBuilder()->hasTable('seller_photos')) {
                $dbPhotos = DB::table('seller_photos')
                    ...
                    // Exclude frozen photos - they have [FROZEN] prefix in photo_caption
                    ->whereRaw('(photo_caption IS NULL OR photo_caption NOT LIKE ? AND photo_caption NOT LIKE ?)', ['[FROZEN]%', '[frozen]%'])
                    ->orderByDesc('is_featured')
                    ->orderBy('sort_order')
                    ->orderByDesc('created_at')
                    ->get(['id', 'photo_url', 'caption', 'photo_caption', 'category', 'is_featured', 'sort_order', 'created_at']);
